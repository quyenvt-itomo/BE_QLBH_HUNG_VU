...
export function generateValidatorFromEntityAuto(entityFilePath: string, outputPath: string) {
...
function extractColumnsFromSource(sourceCode: string): ColumnInfo[] {
  ...
  while ((match = columnRegex.exec(sourceCode)) !== null) {
    ...
    const zodType = mapTypeToZod(propertyType, { ...options, optional: isOptional }, propertyName);
  ...
  while ((propMatch = simplePropertyRegex.exec(sourceCode)) !== null) {
    ...
    const zodType = mapTypeToZod(propertyType, { optional: isOptional }, propertyName);
...
function mapTypeToZod(tsType: string, options: any, fieldName?: string): string {
  ...
  if (options.enumValues && options.enumValues.length > 0) {
    ...
    if (enumValues.length > 0) {
      ...
      if (!isOptional && fieldName) {
        ...
        zodType = "z.string() /* TODO: Replace with proper enum values */";
      }
    }
  } else {
    // Map basic types
...
function generateValidatorContent(className: string, columns: ColumnInfo[]): string {
...
if (require.main === module) {
  ...
  if (args.length >= 2) {
    ...
    console.log("Usage: npx ts-node generate-validator-auto.ts <entity-file-path> <output-path>");
    console.log(
      "Example: npx ts-node generate-validator-auto.ts src/database/entities/User.ts src/modules/user/user.validator.generated.ts"
    );
  }
}
