  ...
  > {
    ...
    for (const doc of stockDocsIn) {
      ...
      const lines = await manager
        .createQueryBuilder()
        .select('sdl."stockQuantity"', "quantity")
        .addSelect(
          isProduction ? "0" : 'COALESCE(pl."unitPrice", 0)',
          "unitCost",
        )
        .addSelect(
          isProduction
            ? "0"
            : 'COALESCE(sdl."stockQuantity", 0) * COALESCE(pl."unitPrice", 0)',
          "amount",
        )
    ...
    for (const doc of transfersIn) {
      const lines = await manager
        .createQueryBuilder()
        .select('wtl."receivedQuantity"', "quantity")
    ...
    for (const doc of transfersOut) {
      const lines = await manager
        .createQueryBuilder()
        .select('wtl."actualQuantity"', "quantity")
        .from("warehouse_transfer_lines", "wtl")
        .where('wtl."transferId" = :refId', { refId: doc.refId })
        .andWhere('wtl."productId" = :productId', { productId })
        .getRawMany();
