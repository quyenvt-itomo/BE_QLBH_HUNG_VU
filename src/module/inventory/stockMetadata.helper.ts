...
export class StockMetadataHelper {
  async updateStockMetadata(
    ...
    await manager
      ...
      .update(productId, {
        stockMetadata: { total: { quantity, value }, byStore },
      });
  }
  ...
  async batchUpdateStockMetadata(
    ...
    for (const id of productIds) await this.updateStockMetadata(id, manager);
  }
  async batchUpdateCascadeToProducts(
    productIds: string[],
    manager: EntityManager,
  ): Promise<void> {
    return this.batchUpdateStockMetadata(productIds, manager);
  }
}
