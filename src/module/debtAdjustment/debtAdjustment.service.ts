...
export class DebtAdjustmentService extends BaseService<DebtAdjustment> {
  ...
  constructor(
    @inject(DEBT_ADJUSTMENT_TYPES.Repository) repository: DebtAdjustmentRepository,
    @inject(PARTNER_TYPES.PartnerRepository) private partnerRepository: PartnerRepository,
  ) { super(); this.repository = repository; }
  async validateBeforeCreate(data: DeepPartial<DebtAdjustment>, manager: EntityManager, req?: RequestContext): Promise<void> {
    ...
    if (data.partnerId && !data.partnerSnapshot) throw new Error("partner.not_found");
  }
}
