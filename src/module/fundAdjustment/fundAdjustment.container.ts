...
export const fundAdjustmentModule = new ContainerModule((bind) => {
  bind(FUND_ADJUSTMENT_TYPES.Repository).to(FundAdjustmentRepository).inSingletonScope();
  bind(FUND_ADJUSTMENT_TYPES.Service).to(FundAdjustmentService).inSingletonScope();
  bind(FUND_ADJUSTMENT_TYPES.Controller).to(FundAdjustmentController).inSingletonScope();
  bind(FUND_ADJUSTMENT_TYPES.Router).to(FundAdjustmentRouter).inSingletonScope();
});
