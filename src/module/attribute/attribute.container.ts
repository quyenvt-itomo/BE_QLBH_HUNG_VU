...
const attributeModule = new ContainerModule((bind) => {
  bind<AttributeController>(ATTRIBUTE_TYPES.AttributeController).to(AttributeController);
  bind<AttributeService>(ATTRIBUTE_TYPES.AttributeService).to(AttributeService);
  bind<AttributeRepository>(ATTRIBUTE_TYPES.AttributeRepository).to(AttributeRepository);
