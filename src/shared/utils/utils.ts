...
export function combineDateAndTime(
  ...
  const time = appDayjs(timeStr); // nếu timeStr là '08:00:00', dayjs sẽ hiểu là hôm nay, lấy giờ/phút/giây
  return appDayjs(dateStr)
    .hour(time.hour())
    .minute(time.minute())
    .second(time.second())
    .millisecond(0)
...
export function isOverdueCheck(
  ...
  const today = appDayjs(actualDate);
  return today.startOf("day").isAfter(appDayjs(timeStr).startOf("day"));
}
...
export function injectStoreIdToArrays<T extends Record<string, any>>(
  ...
  for (const key of STORE_ID_INJECT_KEYS) {
